...
pub fn cargo_command(subcmd: &str, rust_flags: &[&str]) -> Command {
    ...
    let rustflags_envvar = [
        rust_flags,
        &[
            // NOTE: Upstream adds `-C passes=loweratomic` here to lower atomic ops for the guest.
            // Some LLVM builds/toolchains don't support the legacy `loweratomic` pass name, which
            // breaks builds with "unknown pass name 'loweratomic'".
            //
            // For our zkTelemetry guest code, we avoid pulling in atomics; so we omit this flag.
            // If you later add crates that rely on atomics, you may need an alternative strategy
            // (e.g. ensure atomics are not used, or use a toolchain that supports the pass).
