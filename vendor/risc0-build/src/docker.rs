...
fn create_dockerfile(
    ...
    let rustflags = format!(
        // NOTE: Upstream includes `-C passes=loweratomic` here. Some LLVM builds/toolchains don't
        // recognize the legacy `loweratomic` pass name and fail the build.
        "-C link-arg=-Ttext=0x{TEXT_START:08X} -C link-arg=--fatal-warnings",
    );
