...
# Sweep rayon threads for histogram epoch type
#
# Usage:
#   cd zk-Analytics
#   ./scripts/bench_aggr_histogram_thread_sweep.sh
#
# Options (env vars):
...
for threads in $THREADS_LIST; do
  echo "=== Running mode=histogram with threads=$threads ==="
  MODE="histogram" \
    THREADS="$threads" \
    ...
    REPEATS="$REPEATS" \
    OUT_NAME="bench_risc0_aggregator_histogram_threads${threads}.csv" \
    ./scripts/bench_aggregator_sweep.sh
done

echo ""
echo "Histogram thread sweep complete!"
echo "CSV files written to: bench_csv/bench_risc0_aggregator_histogram_threads*.csv"
