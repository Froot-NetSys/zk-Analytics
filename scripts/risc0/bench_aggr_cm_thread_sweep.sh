...
# Sweep rayon threads for cm epoch type
#
# Usage:
#   cd zk-Analytics
#   ./scripts/bench_aggr_cm_thread_sweep.sh
#
# Options (env vars):
...
for threads in $THREADS_LIST; do
  echo "=== Running mode=cm with threads=$threads ==="
  MODE="cm" \
    THREADS="$threads" \
    ...
    REPEATS="$REPEATS" \
    OUT_NAME="bench_risc0_aggregator_cm_threads${threads}.csv" \
    ./scripts/bench_aggregator_sweep.sh
done

echo ""
echo "CM thread sweep complete!"
echo "CSV files written to: bench_csv/bench_risc0_aggregator_cm_threads*.csv"
