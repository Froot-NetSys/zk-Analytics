...
cell() { # dataset nodes mode
  ...
  echo "=================================================================="
  DATASET="$1" NODES="$2" MODE="$3" bash "$ROOT_DIR/scripts/run_distributed_baseline.sh" \
...
python3 "$ROOT_DIR/scripts/build_dist_tables.py" --metrics "$METRICS" --outdir "$ROOT_DIR/results" || true
echo "[matrix] done."
