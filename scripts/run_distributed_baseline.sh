...
FDB_CLUSTER_FILE="$FDB_CLUSTER_FILE" bash "$ROOT_DIR/scripts/reset_fdb.sh" "$FDB_SUBSPACE" >/dev/null 2>&1 || true
...
nohup python3 "$ROOT_DIR/scripts/mem_trace.py" --out "$WORK/mem_query.csv" --summary "$WORK/mem_query.json" \
...
python3 "$ROOT_DIR/scripts/_parse_dist_cell.py" --dataset "$DATASET" --agg-type "$AGG_TYPE" --mode "$MODE" \
