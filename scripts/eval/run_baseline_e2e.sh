...
# (scripts/lib/mem_trace.py, summed across aggregator workers; single process for
...
FDB_CLUSTER_FILE="$FDB_CLUSTER_FILE" FDB_SUBSPACE="$FDB_SUBSPACE" \
  bash "$ROOT_DIR/scripts/setup/reset_fdb.sh" "$FDB_SUBSPACE" >/dev/null 2>&1 || true
...
python3 "$ROOT_DIR/scripts/lib/mem_trace.py" \
...
python3 "$ROOT_DIR/scripts/lib/_parse_e2e_cell.py" \
