...
./scripts/eval/run_zkvm_dev_mode.sh || echo "[phase2] dev sweep had errors (continuing)"

echo "===== [phase2] 2/4 CAIDA prep ====="
if ! compgen -G "$ROOT_DIR/testdata/caida_pcap/caida_txt/*.txt" >/dev/null; then
  ./scripts/setup/prep_caida.sh || echo "[phase2] caida prep failed (continuing)"
fi

echo "===== [phase2] 3/4 native + dev e2e on real datasets ====="
./scripts/eval/run_e2e_native_baseline.sh || echo "[phase2] e2e failed (continuing)"
