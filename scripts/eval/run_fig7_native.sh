...
KAFKA_BROKERS="${KAFKA_BROKERS:-${KAFKA_HOST:-localhost}:9092}"
...
for cfg in ${FIG7_CONFIGS:-samples:samples:samples_sum:1024 histogram:histogram:histogram_p90:1024 cm:cm:cm_topk:8192}; do
  ...
  docker exec kafka kafka-topics --bootstrap-server localhost:9092 --delete --topic "$TOPIC" >/dev/null 2>&1 || true
  sleep 1
  ...
  # This performance baseline intentionally exercises every query type. The
  # deployment policy rejects cm_topk by default, so opt out exactly as the
  # other baseline runner does.
  E2E_TIMING=1 BENCH_PRINT=1 QUERY_NO_PROVE=1 QUERY_POLICY_ENFORCE=0 \
    FDB_CLUSTER_FILE=$FDB_CLUSTER_FILE FDB_SUBSPACE=$SUB \
    HTTP_LISTEN=0.0.0.0:$QPORT "$LBIN/querier" > /tmp/${TAG}_q.log 2>&1 & QPID=$!
  ready=0
  for _ in $(seq 1 60); do
    if curl -s -o /dev/null -m 2 localhost:$QPORT/query -H 'content-type: application/json' \
        -d "{\"type\":\"$query\",\"epochs\":1}"; then
      ready=1
      break
    fi
  ...
  for ec in $COUNTS; do
    ...
    db=$(grep -aoE 'db_ms=[0-9]+' /tmp/${TAG}_q.log | tail -1 | cut -d= -f2 || true)
