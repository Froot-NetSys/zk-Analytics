...
# scripts/eval/run_baseline_e2e.sh for each (dataset, epoch_size, mode) cell, then
...
cell() {  # dataset epoch mode
  ...
  DATASET="$ds" EPOCH_LOGS="$ep" MODE="$md" \
    bash "$ROOT_DIR/scripts/eval/run_baseline_e2e.sh" || echo "[matrix] cell FAILED: $ds/$ep/$md"
}
...
python3 "$ROOT_DIR/scripts/lib/build_baseline_tables.py" \
