...
#
# Usage:
#   ./scripts/run_distributed_e2e.sh [start|stop|status|validate|report]
#
...
usage() {
    ...
    echo "  DATASET_TYPE=synthetic EVENTS=1000000 ./scripts/run_distributed_e2e.sh start"
    echo ""
...
reset_fdb_subspace() {
    ...
    if ssh -o ConnectTimeout=10 "${SSH_USER}@${QUERIER_MACHINE}" \
        "FDB_CLUSTER_FILE='$FDB_CLUSTER_FILE' FDB_RESET_TIMEOUT_SEC='${reset_timeout_sec}' '${REMOTE_PROJECT_DIR}/scripts/reset_fdb.sh' '$FDB_SUBSPACE'"; then
