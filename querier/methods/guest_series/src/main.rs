...
use querier_core::{run_samples_query, SamplesQueryInput, SamplesQueryOutput};
...
fn main() {
    ...
    // side channel; the only signal is the in-band, constant-shape `suppressed`
    // bit. (series == samples output)
