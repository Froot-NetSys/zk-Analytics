...
use zktelemetry_risc0_querier_core::{run_histogram_query, HistogramQueryInput, HistogramQueryOutput};

risc0_zkvm::guest::entry!(main);

fn main() {
    let input: HistogramQueryInput = env::read();
    ...
    let out: HistogramQueryOutput = run_histogram_query(&input).suppress_if_low_support();
    env::commit(&out);
}
