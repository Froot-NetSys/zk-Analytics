...
use core::{hash::Hasher, marker::PhantomData, ops::Deref};
...
{
    /// Inner tree (None for single-element case since merkle_light requires >= 2 leaves)
    tree: Option<merkle::MerkleTree<Node, ShaHasher>>,
    elements: Vec<Element>,
    /// Cached root (used for single-element case where root = hash of single element)
    cached_root: Node,
}

impl<Element> MerkleTree<Element>
where
    Element: Hashable<ShaHasher> + Clone,
{
    pub fn new(elements: Vec<Element>) -> Self {
        assert!(!elements.is_empty(), "MerkleTree requires at least 1 element");

        if elements.len() == 1 {
            // Single element: root is just the hash of that element
            let mut hasher = ShaHasher::default();
            elements[0].hash(&mut hasher);
            let root = hasher.hash();
            Self {
                tree: None,
                elements,
                cached_root: root,
            ...
            let tree = merkle::MerkleTree::<_, ShaHasher>::from_data(elements.iter());
            let root = tree.root();
            Self {
                tree: Some(tree),
                elements,
    ...
    pub fn prove(&self, i: usize) -> Proof<Element> {
        if let Some(ref tree) = self.tree {
            tree.gen_proof(i).into()
        } else {
            // Single element: proof is just the element itself (no siblings)
            // Create a minimal proof with empty path
            Proof::single_element_proof()
        }
    }
}
...
where
    Element: Hashable<ShaHasher>,
{
    /// Create a proof for a single-element Merkle tree.
    /// In this case, the root is just the hash of the element, and the proof path is empty.
    pub fn single_element_proof() -> Self {
        // For single element: lemma = [root], path = []
        // The root will be computed during verification from the element
        proof::Proof::new(vec![], vec![]).into()
    }
...
// Implement Hashable for Digest/Node so we can build Merkle trees from hash values
impl Hashable<ShaHasher> for Digest {
    fn hash(&self, state: &mut ShaHasher) {
    ...
    Element: Hashable<ShaHasher> + Deserialize<'static>,
{
    ...
    pub fn get(&self, index: usize) -> Element {
        ...
        let index_bytes = bincode::serde::encode_to_vec(&(u32::try_from(index).unwrap()), config).unwrap();
        let response = risc0_zkvm::guest::env::send_recv_slice(SYS_VECTOR_ORACLE, &index_bytes);
        let ((value, proof), _): ((Element, Proof<Element>), usize) =
            bincode::serde::borrow_decode_from_slice(response, config).unwrap();
...
#[cfg(test)]
mod test {
    use rust_crypto::{Digest as Sha2Digest, Sha256 as RustSha256};
    ...
    fn random_merkle_tree() -> MerkleTree<u32> {
        ...
        let item_count: usize = rng.gen_range((1 << 10)..(1 << 12));
        let items: Vec<u32> = (0..item_count).map(|_| rng.gen()).collect();
    ...
    fn merkle_proof_serialization_works() {
        ...
        for (index, item) in tree.elements().iter().enumerate() {
            ...
            let proof_bytes = bincode::serde::encode_to_vec(&proof, config).unwrap();
            let (proof_deserialized, _): (Proof<u32>, usize) = bincode::serde::decode_from_slice(&proof_bytes, config).unwrap();
    ...
    fn algorithm_is_consistent_with_sha2() {
        ...
        let reference_hash = RustSha256::digest(test_string);
